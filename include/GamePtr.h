...
#pragma once
   ...
   class GamePtrBase {
     public:
        ...
        virtual void init_ptr() = 0;   // виртуальна€ функци€, что-бы мы могли вызывать ее из массива GamePtrBase дл€ наследников (полиморфизм)
   };

    // суть массива GamePtrBase и вообще создани€ базового класса в том, что нельз€ создать массив объектов/указателей шаблонного класса.
    ...
    class GamePtr : public GamePtrBase
    {
      public:
        GamePtr (int address_, const char* esp_) : GamePtrBase(address_, esp_)
        {
            ...
            // кладем указатель на этот (this) объект наследника в массив базовых классов, пр€мо в к-торе. ¬ массиве будет копи€ указател€ но не важно
        }
        ...
        T* p;   // сам указатель на игровой объект будет здесь. ѕридетс€ обращатьс€ к ним через  some_spell.p
    };

    class PtrsVec {
      public:
        static std::vector<GamePtrBase*>& getVec() {
            static std::vector<GamePtrBase*> ptrs;        // —татический вектор, инициализируетс€ при первом вызове, будет использован в core.cpp
            return ptrs;
        }
    };
