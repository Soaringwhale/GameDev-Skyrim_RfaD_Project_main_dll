...
// -------------------------------------  Action Manager is unused now, usage is through inputWatcher, now commented ----------------------------------
...
class ActionManager 
{
 ...
 public:
    ...
    {
        if ((action.keys.size() == 0 && action.gamepadKeys.size() == 0)    || !action.actionOrIdle)  return false;
